...
detect_notches_pracma_inverted <- function(y, freq_khz, fmin_khz, fmax_khz,
                                          min_peak_separation_khz, peak_height_quantile) {

  # Notches are valleys in y, i.e. peaks in -y
  y_inv <- -y

  band <- freq_khz >= fmin_khz & freq_khz <= fmax_khz & is.finite(y_inv)
  x_band <- freq_khz[band]
  y_band <- y_inv[band]
  ...
  min_peak_height_inv <- as.numeric(
    quantile(y_band, probs = peak_height_quantile, na.rm = TRUE)
  )

  notches <- pracma::findpeaks(
    y_band,
    minpeakheight   = min_peak_height_inv,
    minpeakdistance = min_peak_separation_bins
  )

  if (is.null(notches) || nrow(notches) == 0) numeric(0) else x_band[notches[, 2]]
}
...
for (i in seq_len(n_clicks)) {
  ...
  notches_khz <- detect_notches_pracma_inverted(
    y, freq_khz, fmin_khz, fmax_khz,
    min_peak_separation_khz, peak_height_quantile
  )
