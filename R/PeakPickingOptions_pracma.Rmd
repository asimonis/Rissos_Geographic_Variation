...
#Minimum allowed separation between peaks, in kHz - Original output used 0.6 value
...
#Minimum peak height threshold (quantile-based) - Original output used 0.85 value
min_peak_height_db <- as.numeric(
  quantile(y_band, probs = 0.10, na.rm = TRUE)
)
...
min_peak_height_db <- as.numeric(
  quantile(y_band, probs = 0.85, na.rm = TRUE)
)
...
#Run peak picking for each smoothing type
...
plot(
  ...
  ylab = "Level (dB)",
  xlim = c(19, 50),
  ylim = c(-120, max(y_all, na.rm = TRUE)),
...
label_peaks <- function(peak_df, label_prefix = "") {
  if (nrow(peak_df) == 0) return()
  points(peak_df$peak_frequency_khz, peak_df$peak_level_db, pch = 19, cex = 0.7)
  text(
...
#Table for all smoothing types (explicit binding)

detected_peaks_khz_db_All <- dplyr::bind_rows(
  data.frame(
    UID = click_uid,
    smoothing = "Raw",
    peak_frequency_khz = detected_peaks_khz_db_Raw$freq_khz,
    peak_level_db      = detected_peaks_khz_db_Raw$height_db,
    stringsAsFactors = FALSE
  ),
  data.frame(
    UID = click_uid,
    smoothing = "Tukey",
    peak_frequency_khz = detected_peaks_khz_db_Tukey$freq_khz,
    peak_level_db      = detected_peaks_khz_db_Tukey$height_db,
    stringsAsFactors = FALSE
  ),
  data.frame(
    UID = click_uid,
    smoothing = "Hann5",
    peak_frequency_khz = detected_peaks_khz_db_Hanning$freq_khz,
    peak_level_db      = detected_peaks_khz_db_Hanning$height_db,
    stringsAsFactors = FALSE
  )
)
